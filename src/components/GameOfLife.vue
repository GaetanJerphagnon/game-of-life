<template>
  <div class="toolbar d-flex flex-column justify-content-between">
    ...
    <nav class="navbar navbar-light container bg-secondary justify-content-start">
        ...
        <button data-toggle="tooltip" data-placement="bottom" title="Compute and render only the next generation of life" class="btn btn-info mx-2 my-2 my-sm-0" type="submit" @click="nextTick()">Tick <i class="fa fa-stopwatch"></i></button>
        <button data-toggle="tooltip" data-placement="bottom" title="Reset all cells to dead state" class="btn btn-warning mx-2 my-2 my-sm-0" type="submit" @click="setGrid()">Clear <i class="fa fa-eraser"></i></button>
    ...
    <div class="navbar navbar-light container bg-secondary justify-content-start">
      <span class="text-light">
        ...
        Mousewheel click : <span class="font-weight-bold text-dark"> Death <i class="fa fa-skull-crossbones"></i></span>
      </span>
    </div>
    </nav>
  </div>
...
export default {
  ...
  data(){
    return{
      ...
      isRunning: false,
      interval: null,
      starter:[[31,19],[31,20],[31,21],[32,18],[32,22],[33,17],[34,17],[33,23],[34,23],[35,20],[36,18],[36,22],[37,19],[37,20],[37,21],[38,20]]
    };
  },
  methods:{
    ...
    stopMouseDown(){
      document.querySelector('.game').removeEventListener('mouseup', this.stopMouseDown);
      document.querySelectorAll('.cell').forEach(item => {
    ...
    setNeighbors(cell){
      ...
      for( let yy = y-1; yy<=y+1; yy++ ){
        for(let xx = x-1; xx<=x+1; xx++){
            if(!(xx==x && yy==y) && xx>=0 && yy>=0 && xx<=this.columnsAmount && yy<=this.rowsAmount){
              var neighbor = document.querySelector('[data-coordinates="'+xx+'-'+yy+'"]');
              if(neighbor.classList.contains('cell--alive') != true && neighbor.dataset.tocompute != true){
    ...
    getCell(x,y){
      let cell = document.querySelector('[data-coordinates="'+x+'-'+y+'"]');
      return cell;
    },
    ...
    nextStateCell(cell){
      ...
      let y = parseInt(cell.dataset.y);
      let cpt = 0;
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y-1][x-1]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y-1][x]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y-1][x+1]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y][x-1]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y][x+1]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y+1][x-1]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y+1][x]) {cpt++}
      if(x>0 && y>0 && x<this.columnsAmount-1 && y<this.rowsAmount-1 && this.grid[y+1][x+1]) {cpt++}
  ...
  mounted(){
    this.setGrid();
    document.querySelector('.game').addEventListener('mousedown', this.onMouseDown); 
  }
  /* props: {
    msg: String
  } */
